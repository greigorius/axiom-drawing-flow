...
const match = (page, f) => {
  ...
  if (f.url)      return (p?.url ?? null) === f.url.equals;
...
(async () => {
  ...
  updates.length = 0;
  r = await call("POST /api/df/cr-ingest", { body: { filePath: `${R}/24-367/Client Comments/MC_260910_${DWG}_P02.pdf`, shareLink: "https://db/x" } });
  ...
  assert.strictEqual(subUpd["Comment Paths"].rich_text[0].text.content, `Drawing Submissions/24-367/Client Comments/MC_260910_${DWG}_P02.pdf`);
  ...
  r = await call("POST /api/df/cr-ingest", { body: { filePath: `${R}/24-367/Client Comments/Reviewed/R_MC_260910_${DWG}_P02.pdf` } });
  ...
  submissions = [issued("subI", "S5", "P02", { "DM Action": sel("Review Comments"),
    "Comment Paths": rt(`Drawing Submissions/24-367/Client Comments/MC_260910_${DWG}_P02.pdf\nDrawing Submissions/24-367/Client Comments/AR_260911_${DWG}_P02.pdf`),
    "Dropbox Path": { url: `Drawing Submissions/24-367/Approved/003_S5_P02_${DWG}.pdf` } })];
  webhooks.length = 0; updates.length = 0;
  ...
  assert.strictEqual(mv.moves[0].toFolderParent, `${R}/24-367/Client Comments`); assert.strictEqual(mv.moves[0].toFolderName, "Reviewed");
  ...
  assert.match(lsUpd["Comment Paths"].rich_text[0].text.content, /Client Comments\/Reviewed\/R_MC_260910/);
  ...
  submissions[0].properties["Comment Paths"] = rt(`Drawing Submissions/24-367/Client Comments/Reviewed/R_MC_260910_${DWG}_P02.pdf`);
  webhooks.length = 0;
  ...
  submissions = [issued("subC", "A4.5", "C01", { "Submission": title(`24-367-003_${DWG}_A4.5_R1`),
    "Dropbox Path": { url: `Drawing Submissions/24-367/Approved/003_A4.5_C01_${DWG}.pdf` } })];
  webhooks.length = 0; updates.length = 0;
  ...
  assert.strictEqual(mv.moves[0].toFolderParent, `${R}/24-367`); assert.strictEqual(mv.moves[0].toFolderName, "Grade Returns");
  assert.match(mv.moves[0].newFilename, new RegExp(`^003_A4\\.5_C01_${DWG}_Rejected_\\d{6}\\.pdf$`));
  assert.match(updates[0].properties["Dropbox Path"].url, /^Drawing Submissions\/24-367\/Grade Returns\/003_A4\.5_C01_/);
  webhooks.length = 0;
  r = await call("PATCH /api/df/submissions/:id/log-status", { params: { id: "subC" }, body: { grade: "Approved" } });
  assert.strictEqual(hook("move-files"), undefined);
  ok("A4.5 Rejected → Grade Returns/{Item}_{Stage}_{Rev}_{DrawingNo}_Rejected_{YYMMDD}.pdf; Approved stays put");
  ...
  submissions = [
    { id: "g1", properties: { "Status": sel("Graded"), "DT Notified": { checkbox: false }, "Stage": sel("S5"), "Client Grade": sel("B"), "Revision": sel("P02"),
      ...
      "Comment Paths": rt(`Drawing Submissions/24-367/Client Comments/Reviewed/R_MC_260910_${DWG}_P02.pdf`) } },
    { id: "g2", properties: { "Status": sel("Graded"), "DT Notified": { checkbox: false }, "Stage": sel("A4.5"), "Client Grade": sel("Rejected"), "Revision": sel("C01"),
      ...
      "Dropbox Path": { url: `Drawing Submissions/24-367/Grade Returns/003_A4.5_C01_${DWG}_Rejected_260910.pdf` } } },
  ];
  webhooks.length = 0;
  r = await call("POST /api/df/send-grade-emails", { body: {} });
  const gs = hook("grade-summary");
  const heads = gs.folderBlocks.map((f) => f.folderHtml);
  assert.ok(heads.includes("<strong>24-367/Client Comments/Reviewed</strong>"), heads.join(" | "));
  assert.ok(heads.includes("<strong>24-367/Grade Returns</strong>"), heads.join(" | "));
  assert.ok(gs.folderBlocks.some((f) => /R_/.test(f.drawingsHtml)) && gs.folderBlocks.some((f) => /\{Item\}_\{Stage\}/.test(f.drawingsHtml)));
  ok("grade email: S5 → Client Comments/Reviewed, A4.5 → Grade Returns");
  ...
  r = await call("POST /api/df/stage-upload", { body: { filePath: `${R}/24-367/Approved/003_S4_P01_A-101.dwg` } });
  assert.strictEqual(r.json.updated, 1); assert.match(seen[0], /"S4"/);
  r = await call("POST /api/df/stage-upload", { body: { filePath: `${R}/24-367/Approved/A-101.dwg` } });
